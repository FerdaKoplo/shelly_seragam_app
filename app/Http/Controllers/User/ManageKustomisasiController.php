...
const DETAIL_KOMBINASI  = 'Jumlah Kombinasi Kain';
    ...
    public function store(Request $request)
    {
        ...
        try {
            foreach ($request->input('sections') as $sec) {
                if (ProdukKustom::where('spesifikasi_khusus', $sec['name'])->exists()) {
                    DB::rollBack();
            ...
            DB::commit();
            return redirect()->route('manage.kustom')
                ->with('success', 'Produk kustom berhasil dibuat.');
 
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Store kustom gagal', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
    ...
    public function edit($id)
    {
        ...
        // Load all kustom records for the same produk (all sections of this product)
        $kustoms = ProdukKustom::with('produk.detailProduks.pilihanDetails')
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            foreach ($request->input('sections') as $sec) {
                ...
                if ($kustomId) {
                    $kustom = ProdukKustom::with('produk.detailProduks.pilihanDetails')
                        ->findOrFail($kustomId);
            ...
            DB::commit();
            return redirect()->route('manage.kustom')
                ->with('success', 'Produk kustom berhasil diperbarui.');
 
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Update kustom gagal', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('manage.kustom')
                ->with('success', 'Produk kustom berhasil dihapus.');
    ...
    private function saveDetails(int $produkId, $existing, array $sec): void
    {
        ...
        $this->upsertMulti($produkId, $existing, self::DETAIL_KOMBINASI,
            array_map('strval', $counts));
 
        $this->upsertMulti($produkId, $existing, self::DETAIL_BORDIR,
            array_map('strval', $bordirs));
