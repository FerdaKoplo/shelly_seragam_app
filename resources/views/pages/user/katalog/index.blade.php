...
@section('content')
    ...
    <div x-data class="flex gap-10 md:gap-16 flex-col justify-center items-center px-4 md:px-10 max-w-[1600px] mx-auto">
        
        <div class="flex flex-col md:flex-row items-center gap-4 md:gap-7 w-full">
            ...
            <form method="GET" action="{{ route('manage.katalog') }}" class="flex-1 relative w-full">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 md:gap-8 w-full">
            @forelse ($katalog as $item)
                ...
                <div data-cy="produk-card" class="bg-white shadow-md rounded-2xl relative group  transition-all duration-300 flex flex-col border border-gray-100">
                    ...
                    <div class="w-full aspect-[4/5] overflow-hidden rounded-t-2xl bg-[#F5F5F5] relative">
                        <div class="{{ $isArchived ? 'opacity-40 grayscale' : '' }} w-full h-full">
                    ...
                    <div class="p-4 flex flex-col justify-between">
                        <div class="{{ $isArchived ? 'opacity-50' : '' }} mb-3">
                            <h3 data-cy="produk-nama" class="font-bold text-base text-gray-900 line-clamp-2 leading-tight min-h-[2.5rem]">
                                {{ $item->produk->nama_produk ?? 'Nama Produk' }}
                            </h3>
                            <span class="inline-block mt-1 px-2 py-0.5 rounded bg-gray-100 text-[10px] font-medium text-gray-600">
                                #{{ $item->kategori ?? 'Kategori' }}
                            </span>
                        </div>

                        <div class="flex items-center justify-between border-t border-gray-50 pt-3">
                            <p class="font-bold text-lg text-black {{ $isArchived ? 'opacity-50' : '' }}">
                                ...
                                @auth
                                    @if(auth()->user()->role === 'Admin')
                                        <button type="button" data-cy="btn-delete-produk"
                                            ...
                                            class="text-gray-400 hover:text-red-600 transition" title="Hapus Produk">
            ...
            @empty
                <div class="col-span-full flex flex-col items-center justify-center min-h-[40vh] font-medium text-lg text-gray-400">
    ...
    <a href="{{ route('manage.katalog.create') }}">
        <button id="btn-tambah-produk-baru"
            class="z-50 fixed bottom-6 md:bottom-8 flex font-inter items-center gap-2 md:gap-3 right-4 md:right-8 px-5 bg-white border border-black text-black py-3 rounded-full shadow-2xl hover:bg-gray-50 transition active:scale-95">
            <p class="text-sm md:text-base font-bold">
                <span class="hidden sm:inline">Tambahkan</span> Produk Baru
            </p>
            <svg width="24" height="24" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" class="md:w-[30px] md:h-[30px]">
                <path d="M21.4891 13.1995L16.1213 13.1995V7.83171H13.199V13.1995L7.83124 13.1995V16.1217L13.199 16.1217V21.4895H16.1213V16.1217L21.4891 16.1217V13.1995ZM25.0227 4.29808C19.2922 -1.4324 10.0281 -1.4324 4.29762 4.29808C-1.43286 10.0286 -1.43286 19.2927 4.29762 25.0232C10.0281 30.7536 19.2922 30.7536 25.0227 25.0232C30.7532 19.2927 30.7532 10.0286 25.0227 4.29808ZM6.37013 22.9506C1.80025 18.3808 1.80025 10.9405 6.37013 6.37059C10.94 1.80071 18.3803 1.80071 22.9502 6.37059C27.5201 10.9405 27.5201 18.3808 22.9502 22.9506C18.3803 27.5205 10.94 27.5205 6.37013 22.9506Z" fill="#323232" />
            </svg>
        </button>
    </a>
