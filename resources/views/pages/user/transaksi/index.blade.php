...
@section('content')
    <div x-data="{
            ...
            <div class="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4 mb-6">
                ...
                <div class="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                    ...
                    @if(request('filter') || request('search'))
                        <a href="{{ route('manage.transaksi') }}" class="px-4 py-2 text-sm text-red-500 hover:underline flex items-center shrink-0">Reset</a>
                    @endif
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse($transaksis as $trx)
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition group">
                                <td class="py-3 px-4 text-gray-500">#TRX{{ str_pad($trx->transaksi_id, 3, '0', STR_PAD_LEFT) }}</td>
                                <td class="py-3 px-2 font-semibold text-gray-900">{{ $trx->nama_customer }}</td>
                                <td class="py-3 px-2 text-gray-600">{{ $tipe }}</td>
                                <td class="py-3 px-2">
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-bold {{ $statusBayar == 'Lunas' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                        {{ $statusBayar }}
                                    </span>
                                </td>
                                <td class="py-3 px-2 text-gray-600">{{ $statusKirim }}</td>
                                <td class="py-3 px-4 text-right">
                                    <button @click="modalOpen = true; selectedTrx = {{ $trx->transaksi_id }};" 
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-12 text-center text-gray-400 italic">Tidak ada data transaksi ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="modalOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4" x-cloak style="display: none;">
            @foreach($transaksis as $trx)
                ...
                <div x-show="selectedTrx === {{ $trx->transaksi_id }}" @click.away="modalOpen = false; selectedTrx = null"
                    ...
                    <div class="px-6 md:px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-white shrink-0">
                        ...
                        <button @click="modalOpen = false; selectedTrx = null" class="text-gray-400 hover:text-red-500 hover:bg-red-50 p-2 rounded-full transition-colors">
                    ...
                    <div class="p-4 md:p-8 flex-1 overflow-y-auto bg-gray-50/50 custom-scrollbar">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div class="space-y-6">
                                <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                                    ...
                                    <div class="space-y-3 text-sm">
                                        <div class="grid grid-cols-3">
                                            ...
                                            <span class="font-semibold text-gray-900 col-span-2">{{ $trx->nama_customer }}</span>
                                        </div>
                                        <div class="grid grid-cols-3">
                                            <span class="text-gray-500">Alamat</span> 
                                            <span class="font-medium text-gray-700 col-span-2 leading-relaxed">{{ $trx->alamat_customer }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                @if($isKustom)
                                    ...
                                    <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                                        <h3 class="text-sm font-bold text-gray-800 mb-4 uppercase tracking-wider flex items-center gap-2">
                                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
                                            Bukti Pembayaran (Kustom)
                                        </h3>
                                        
                                        @if($paymentAttachment)
                                            {{-- Enhanced Attachment Box --}}
                                            <div class="bg-green-50/50 border border-green-200 rounded-lg p-4 flex flex-col gap-3">
                                                ...
                                                <div class="bg-white border border-green-100 rounded-md p-2.5 flex items-center justify-between shadow-sm group">
                                                    <div class="flex items-center gap-3 overflow-hidden pr-3">
                                                        ...
                                                        <div class="flex flex-col min-w-0">
                                                            <span class="text-[11px] font-semibold text-gray-800 truncate">{{ basename($paymentAttachment->path) }}</span>
                                ...
                                <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                                    <h3 class="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2 uppercase tracking-wider">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                                        Update Status
                                    </h3>
                                    <form action="{{ route('manage.transaksi.update', $trx->transaksi_id) }}" method="POST" id="form-{{$trx->transaksi_id}}">
                                        ...
                                        <div class="space-y-4 text-sm">
                                            <div class="flex justify-between items-center">
                                                ...
                                                <span class="px-2.5 py-0.5 rounded-full text-[11px] font-bold {{ $statusBayar == 'Lunas' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                    {{ $trx->status }}
                                                </span>
                                            </div>
                                        ...
                                        <div class="flex-1 flex flex-col justify-between">
                                            <div class="space-y-5 text-sm text-gray-700">
                                                <div class="grid grid-cols-3">
                                                    ...
                                                    <span class="col-span-2 font-semibold text-gray-900">{{ $kustom->tipe_kustom }}</span>
                                                </div>
                                                <div class="grid grid-cols-3">
                                                    <strong class="col-span-1 text-gray-500">Kain / Catatan</strong> 
                                                    <span class="col-span-2 font-medium text-gray-700 leading-relaxed">{{ $kustom->catatan ?? '-' }}</span>
                                                </div>
                                                <div class="grid grid-cols-3">
                                                    <strong class="col-span-1 text-gray-500">Jumlah</strong> 
                                                    <span class="col-span-2 font-bold text-gray-900">{{ $kustom->quantity }} pcs</span>
                                                </div>
                                    ...
                                    @else
                                        <div class="space-y-4 flex-1">
                                            @foreach($trx->produkTransaksis as $pt)
                                                <div class="flex gap-4 p-4 border border-gray-100 rounded-xl bg-gray-50/50 hover:bg-white hover:shadow-md hover:border-gray-200 transition-all">
