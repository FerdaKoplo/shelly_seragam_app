            ...
            <form action="{{ route('manage.kustom.store') }}" method="POST" x-data="kustomForm($dispatch)"
                        ...
                        <div x-show="sec.showKombinasi" x-transition:enter="transition ease-out duration-200"
                            ...
                            <div class="flex gap-3 flex-wrap items-center">
                                @foreach($allCounts as $n)
                                    <button type="button" data-cy="toggle-kombinasi-{{ $n }}"
                                        @click="toggleCount(sIdx, {{ $n }})" :class="sec.enabledCounts.includes({{ $n }})
                                    ...
                                    <div class="border border-black rounded-lg p-4 w-[280px]"
                                        ...
                                        <div class="grid grid-cols-3 gap-2">
                                            @foreach($materials as $mat)
                                                <span
                                                    class="px-1 py-1.5 text-[11px] rounded border border-gray-300 bg-white text-gray-800 text-center">
                                                    {{ $mat }}
                                                </span>
                                            @endforeach
                                        </div>
                        ...
                        <div x-show="sec.showBordir" x-transition:enter="transition ease-out duration-200"
                            ...
                            <div class="flex gap-3 flex-wrap items-center">
                                @foreach($allBordirs as $n)
                                    <button type="button" @click="toggleBordir(sIdx, {{ $n }})" :class="sec.enabledBordirs.includes({{ $n }})
                                                                    ? 'bg-black text-white border-black'
                                                                    : 'bg-white text-black border-black hover:bg-gray-50'"
                ...
                <div class="flex flex-col gap-4 mt-4">
                    ...
                    <div x-show="showUpload"
                        ...
                        <div
                            class="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-gray-300 rounded-xl py-10 bg-gray-50">
