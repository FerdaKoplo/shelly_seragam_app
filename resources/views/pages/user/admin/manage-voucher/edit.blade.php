...
<div class="px-4 md:px-6 pb-12">
    ...
    <form data-cy="edit-voucher-form" action="{{ route('manage.voucher.update', $voucher->id) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-28">
        @csrf
        @method('PUT')
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="relative">
                    <select data-cy="voucher-type-select" name="jenis_voucher" class="w-full border border-black rounded-md px-4 py-3 text-sm md:text-base focus:outline-none focus:ring-1 focus:ring-black appearance-none bg-white" required>
        ...
        <div class="lg:col-span-5 space-y-6 md:space-y-10">
            <div class="flex flex-col gap-5">
                ...
                <div class="flex flex-col gap-2">
                    <label class="font-bold text-gray-900 text-sm md:text-base">Kode Voucher <br class="sm:hidden"/><span class="text-xs md:text-sm font-normal text-gray-500">(Kosongkan untuk generate otomatis)</span></label>
                    <div class="relative">
                ...
                <div class="flex flex-col gap-2 mt-2">
                    ...
                    <div class="relative">
                        <input data-cy="voucher-discount-input" onkeydown="if(event.key === '-') event.preventDefault();" type="number" name="nilai_diskon" value="{{ old('nilai_diskon', $voucher->nilai_diskon) }}" placeholder="0"
                            class="w-full border border-black rounded-md px-4 py-3 text-2xl md:text-4xl font-normal placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-black"
                            required>
                        <span class="absolute top-2 right-2 text-red-500 text-xl">*</span>
                    </div>
                </div>
