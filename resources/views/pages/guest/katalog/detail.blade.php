...
@section('content')
    ...
    <div data-cy="product-detail" class="max-w-full mx-auto px-32 py-8" x-data="{
        ...
        <div class="flex items-center gap-4 mb-8">
            <a data-cy="btn-back" href="{{ route('katalog') }}"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mx-auto items-start">
            ...
            <div class="lg:col-span-1 flex flex-col space-y-10">
                @forelse($item->fotos as $foto)
                    ...
                    <button type="button" data-cy="thumbnail-{{ $loop->index }}" @click="activeImage = '{{ $url }}'"
                ...
                @empty
                    ...
                    @foreach(range(1, 4) as $i)
                        ...
                        <button type="button" data-cy="thumbnail-fallback-{{ $i }}" @click="activeImage = '{{ $thumbnailUrl }}'"
                            class="border transition-all duration-200 aspect-[3/4] overflow-hidden"
            ...
            <div class="lg:col-span-6 bg-gray-50 flex items-center justify-center rounded-sm overflow-hidden min-h-[500px]">
                <img data-cy="main-image" :key="activeImage" :src="activeImage" alt="{{ $item->produk->nama_produk }}"
            ...
            <div class="lg:col-span-4 mx-auto">
                <h1 data-cy="product-name" class="text-4xl font-normal text-gray-900 mb-2">{{ $item->produk->nama_produk }}
                </h1>
                <p data-cy="product-price" class="text-5xl font-bold mb-2">Rp{{ number_format($item->harga, 0, ',', '.') }}
                </p>
                @if ($isPreOrder)
                ...
                <div class="mb-6">
                    @if($sizeOptions->isNotEmpty())
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <button data-cy="btn-size-guide" @click="$dispatch('open-modal', 'modal-panduan-ukuran')"
                                class="text-xs flex items-center text-gray-500 hover:text-black">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M3 10h18M7 15h1m4 0h1m4 0h1"></path>
                                </svg>
                                Panduan Ukuran
                            </button>
                        </div>
                        <div class="grid grid-cols-3 gap-2">
                            @foreach($sizeOptions as $opt)
                                ...
                                <label class="cursor-pointer">
                                    <input type="radio" name="size" data-cy="size-{{ $sz }}" value="{{ $sz }}"
                                        x-model="selectedSize" class="hidden peer">
                                    <div
                                        class="border border-gray-200 py-2 text-center rounded-lg peer-checked:bg-black peer-checked:text-white transition">
                                        {{ $sz }}</div>
                                </label>
                            @endforeach
                        </div>
                    ...
                    @if($colorOptions->isNotEmpty())
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            @foreach($colorOptions as $opt)
                                ...
                                <label class="cursor-pointer">
                                    <input type="radio" name="color" value="{{ $hex }}" x-model="selectedColor"
                                        data-cy="color-{{$hex}}" class="hidden peer">
        ...
        <div class="mt-16 max-w-4xl">
            <p data-cy="product-sold" class="text-gray-500 mb-2">Terjual :
                {{ number_format((int) ($sold ?? 0), 0, ',', '.') }}</p>
